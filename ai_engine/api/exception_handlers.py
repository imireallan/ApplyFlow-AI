...
class InvalidFileTypeError(HTTPException):
    def __init__(self, detail: str = "Invalid file type. Only PDF files are supported."):
        super().__init__(status_code=status.HTTP_400_BAD_REQUEST, detail=detail)


class MissingCVIdError(HTTPException):
    def __init__(self, detail: str = "Missing CV Id"):
        super().__init__(status_code=status.HTTP_400_BAD_REQUEST, detail=detail)
...
class VectorStoreError(HTTPException):
    def __init__(
        ...
        if log_message:
            ...
            elif "connection" in log_message.lower() or "connect" in log_message.lower():
